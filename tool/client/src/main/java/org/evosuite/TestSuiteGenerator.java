...
public class TestSuiteGenerator {
    ...
    public TestGenerationResult generateTestSuite() {
        ...
        if (Properties.NPETEST_ON) {
            // LoggingUtils.getEvoLogger().info("CHANGE CRITERION");
            // Properties.CRITERION = Properties.NPETEST_CRITERION;
        }
        ...
        LoggingUtils.getEvoLogger().info("COVERAGE INFORMATION");
        CoverageCriteriaAnalyzer.reportCoverage(testCases, Properties.Criterion.LINE);
...
//			test.addStatement(loadClassStmt);
    ...
    protected void postProcessTests(TestSuiteChromosome testSuite) {
        ...
        // LoggingUtils.getEvoLogger().info("CHECK  COVERAGE GOALS");
        for (TestFitnessFactory<?> ff : getFitnessFactories()) {
            // if (ff instanceof BranchCoverageFactory) {
            //     LoggingUtils.getEvoLogger().info("BRANCH COVERAGE");
            //     LoggingUtils.getEvoLogger().info(Integer.toString(ff.getCoverageGoals().size()));
            // } else if (ff instanceof LineCoverageFactory) {
        ...
        for (TestFitnessFunction f : testSuite.getCoveredGoals()) {
            testSuite.getNumOfCoveredGoals(f);
        ...
        if (!Properties.ANALYSIS_CRITERIA.isEmpty()) {
            LoggingUtils.getEvoLogger().info("ANALYSIS_CRITERIA??" + Properties.ANALYSIS_CRITERIA);
        ...
        if (Properties.NPETEST_ON) {
            logger.debug("AFTER ASSERTIONS: " + Integer.toString(testSuite.getTests().size()));
        ...
        if (Properties.NPETEST_ON) {
            logger.debug("AFTER COMPILATION CHECK: " + Integer.toString(testSuite.getTests().size()));
            logger.debug(testSuite.toString());
        }
    }
    ...
    private TestSuiteChromosome generateTests() {
        ...
        List<TestCase> tmpTests = testSuite.getTests();
        
        LoggingUtils.getEvoLogger().info("* # of TestSuite: " + NumberFormat.getIntegerInstance().format(tmpTests.size()));
    ...
    public static TestGenerationResult writeJUnitTestsAndCreateResult(TestSuiteChromosome testSuite, String suffix) {
        ...
        if (Properties.NPETEST_ON) {
            logger.debug("WRITE JUNIT TESTS");
            for (TestCase tc : tests) {
                logger.debug(tc.toCode());
