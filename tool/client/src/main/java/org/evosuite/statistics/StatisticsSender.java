...
public class StatisticsSender {
     ...
     * @param individual
     */
    public static <T extends Chromosome<T>> void sendIndividualToMasterNPETest(T individual) throws IllegalArgumentException {
        if (individual == null) {
            throw new IllegalArgumentException("No defined individual to send");
        }
        if (!Properties.NEW_STATISTICS)
        ...
        ClientServices.<T>getInstance().getClientNode().updateStatisticsNPETest(individual);
    }
     ...
     * and then send it to Master
     */
    public static void executedAndThenSendIndividualToMasterNPETest(TestSuiteChromosome testSuite) throws IllegalArgumentException {
        ...
        sendIndividualToMasterNPETest(testSuite);
