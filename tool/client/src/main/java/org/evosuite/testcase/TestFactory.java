...
import org.evosuite.testcase.mutation.GuidedRandomInsertion;
...
//					throw new ConstructionFailedException("Cannot currently instantiate type "+type);
    ...
    public boolean insertRandomCall(TestCase test, int position) {
        ...
        try {
            ...
            if (o == null) {
                logger.warn("InsertRandomCall: Have no target methods to test");
                return false;
            } else if (o.isConstructor()) {
    ...
    // Inserting the NPE-prone methods only
    public boolean insertGuidedTargetCall(TestCase test, int position) {
        int previousLength = test.size();
        String name = "";
        currentRecursion.clear();
        logger.debug("Inserting random call at position {}", position);
        ...
        try {
            ...
            GenericAccessibleObject<?> o = TestCluster.getInstance().getGuidedTestCall(test);
            
            if (o == null) {
                logger.warn("insertGuidedTargetCall: Have no target methods to test");
                return false;
            } else if (o.isMethod()) {
                GenericMethod m = (GenericMethod) o;
                logger.debug("Adding method call {}", m.getName());
                ...
                String methodKey = ClassInfo.getInstance().getMethodKey(m.toString());
                test.setMUT(methodKey);
                test.setTriggeredNPE(0);
    ...
    public boolean insertNonTargetRandomCall(TestCase test, int position) {
        int previousLength = test.size();
        String name = "";
        currentRecursion.clear();
        logger.debug("Inserting random call at position {}", position);
        try {
            ...
            GenericAccessibleObject<?> o = TestCluster.getInstance().getNonTargetTestCall(test);
            
            if (o == null) {
                logger.warn("insertNonTargetRandomCall: Have no target methods to test");
                return false;
            } else if (o.isConstructor()) {
    ...
    public int insertTargetMethod(TestCase test, int lastPosition) {
        GuidedRandomInsertion rs = new GuidedRandomInsertion();
        return rs.insertTargetMethod(test, lastPosition);
    }
    

    public int insertMethodForInit(TestCase test, int lastPosition) {
        GuidedRandomInsertion rs = new GuidedRandomInsertion();
        return rs.insertTargetMethod(test, lastPosition);
    }

    public int insertGuidedRandomStatement(TestCase test, int lastPosition) {
        GuidedRandomInsertion rs = new GuidedRandomInsertion();
        // return rs.insertStatement(test, lastPosition);
        return rs.insertNonTargetStatement(test, lastPosition);
    }
