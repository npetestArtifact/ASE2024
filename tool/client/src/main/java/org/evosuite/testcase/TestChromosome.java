...
public final class TestChromosome extends AbstractTestChromosome<TestChromosome> {
    ...
    private float calculateScore() {
        ...
        for (MethodCall m : this.getLastExecutionResult().getTrace().getMethodCalls()) {
            
            String executedMethod = m.className + "." + m.methodName;
            if (executedMethod.contains("<init>") || executedMethod.equals(".")) {
                score += 0.1f;
                continue;
            }

            String tmpStr = ClassInfo.getInstance().getExecutedMethodKey(executedMethod);

            score += MethodInfo.getInstance().getScore(tmpStr);            
        }
    ...
    public void guidedMutate() {
        boolean changed = false;
        mutationHistory.clear(); 
        // LoggingUtils.getEvoLogger().info("CURRENT MUT FOR THE TEST: " + test.getMUT());
        // LoggingUtils.getEvoLogger().info(test.toCode());

        ClassInfo.getInstance().setCurrentMethod(test.getMUT());

        double rd = Randomness.nextDouble();

        // Change
        if (rd <= 0.5 && test.size() > 3) {
            logger.debug("Mutation: change");            
            if (guidedMutationChange())
                changed = true;
        } 
        // else {
        else if (rd <= 0.9) {
            logger.debug("Mutation: insert");
            if (guidedMutationInsert())
                changed = true;
        } 
        else {
            if (guidedMutationDelete())
                changed = true;
        }
    ...
    // We want to leave the last statement (MUT)
    private int getGuidedMutatableStatement() {
        final ExecutionResult result = getLastExecutionResult();
        final int size = test.size();
        ...
        if (result != null && !result.noThrownExceptions()) {
            ...
            return test.size() >= 2 ? test.size() - 2 : test.size() - 1;
        }
    }
    ...
    private boolean guidedMutationDelete() {
        ...
        boolean changed = false;
        int lastMutableStatement = getGuidedMutatableStatement();
        double pl = 1d / (lastMutableStatement + 1);
        TestFactory testFactory = TestFactory.getInstance();
    ...
    List<Statement> resultStmt = new ArrayList<>();

    private void getTargetStmts() {
        final ExecutionResult result = getLastExecutionResult();
        final int size = test.size();
        int last = 0;
        ...
        if (result != null && !result.noThrownExceptions()) {
            ...
            // It may happen that pos > size() after statements have been deleted.
            last = pos >= size ? size - 1 : pos;
        } else {
            
            last = test.size() - 1;
        }

        resultStmt = new ArrayList<>();
        List<VariableReference> targetVar = new ArrayList<>();

        HashSet<Integer> targetParams = MethodInfo.getInstance().getTargetParamIndex(test.getMUT());

        if (targetParams.isEmpty()) {
            targetVar = test.getStatement(last).getUniqueVariableReferences();
        } else {
            List<VariableReference> tmp = test.getStatement(last).getUniqueVariableReferences();

            for (int i : targetParams) {
                targetVar.add(tmp.get(i + 1));
            }

            targetVar.add(tmp.get(tmp.size() - 1));
        }

        for (int i = last - 1; i >= 0; i--) {
            List<VariableReference> tmpVars = test.getStatement(i).getUniqueVariableReferences();

            tmpVars.retainAll(targetVar);

            if (tmpVars != null && !tmpVars.isEmpty()) {
                targetVar.addAll(test.getStatement(i).getUniqueVariableReferences());
                resultStmt.add(test.getStatement(i));
            }
        }
    }

    private Statement getMutateTargetStmt() {
        final ExecutionResult result = getLastExecutionResult();
        final int size = test.size();
        int last = 0;
        ...
        if (resultStmt.isEmpty()) {
            return test.getStatement(Randomness.nextInt(last));
        }
        else {
            return resultStmt.get(Randomness.nextInt(resultStmt.size()));
        }
    }

    private boolean guidedMutationChange() {
        boolean changed = false;

        if (Randomness.nextDouble() < 0.3) {
            int lastMutatableStatement = getGuidedMutatableStatement();
            double pl = 1d / (lastMutatableStatement + 1);
            TestFactory testFactory = TestFactory.getInstance();
            ...
            Statement statement = getMutateTargetStmt();

            while (statement.isReflectionStatement()) {
                if (cnt > 10) return false;

                resultStmt.remove(statement);

                statement = getMutateTargetStmt();
                cnt++;
            }   
     ...
     * @return
     */
    public boolean guidedMutationInsert() {
        boolean changed = false;
        final double ALPHA = Properties.P_STATEMENT_INSERTION; //0.5;
        int count = 0;
        TestFactory testFactory = TestFactory.getInstance();
        ...
        while (Randomness.nextDouble() <= Math.pow(ALPHA, count)
            ...
            int targetStatement = getGuidedMutatableStatement();
            int position = 0;

            if (Randomness.nextDouble() < 0.7) 
                position = testFactory.insertGuidedRandomStatement(test, targetStatement);
            else   
                position = testFactory.insertRandomStatement(test, targetStatement);
    ...
    public int getTriggeredNPE() {
        return this.test.getTriggeredNPE();
    }

    public void setTriggeredNPE(int pos) {
        this.test.setTriggeredNPE(pos);
    }
