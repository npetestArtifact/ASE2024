...
public class MethodStatement extends EntityWithParametersStatement {
    ...
    public boolean mutate(TestCase test, TestFactory factory) {
        ...
        if (Properties.NPETEST_ON && parameters.size() > 0) {
            int numParameter = 0;

            if (Randomness.nextDouble() <= 0.5) {
                for (numParameter = 0; numParameter < parameters.size(); numParameter++) {
                ...
                String methodKey = ClassInfo.getInstance().getMethodKey(this.method.toString());
                numParameter = MethodInfo.getInstance().chooseParams(methodKey);
        
                if (mutateParameter(test, numParameter)) changed = true;
            }
        } else {
