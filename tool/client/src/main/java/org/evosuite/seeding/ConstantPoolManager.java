...
public class ConstantPoolManager {
    ...
    private HashMap<String, ConstantPool> NPEPools = new HashMap<>();

    // private ConstantPool NPEPools = new NPEConstantPool();
...
//		double p = 1d / probabilities.length;
    ...
    // TODO: update NPE Constant Pools.
    // mapping to methods? not?
    public void updateNPEConstantPools(String targetMethod, Object value) {
        ConstantPool tmpPool = null;
        if (!NPEPools.containsKey(targetMethod)) {
            tmpPool = new NPEConstantPool();
        } else {
            tmpPool = NPEPools.get(targetMethod);
        }
        tmpPool.add(value);
        NPEPools.put(targetMethod, tmpPool);
    }
    ...
    public ConstantPool getConstantPool() {
        ...
        if (!NPEPools.isEmpty() && Properties.NPETEST_ON && Randomness.nextDouble() < 0.5) {
            String targetMethod = ClassInfo.getInstance().getCurrentMethod();

            if (targetMethod != null && NPEPools.containsKey(targetMethod)) {
                // LoggingUtils.getEvoLogger().info("NPE CONSTANT POOL STARTS");
                return NPEPools.get(targetMethod);
            }
        }

        // LoggingUtils.getEvoLogger().info("NOT NPE CONSTANT POOL");
        for (int i = 0; i < probabilities.length; i++) {
            k += probabilities[i];
            if (p < k) {
                return pools[i];
