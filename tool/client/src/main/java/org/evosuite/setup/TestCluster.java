...
import org.evosuite.analysis.WeightedCollection;
...
public class TestCluster {
    ...
    private final static Set<GenericAccessibleObject<?>> NPEMethods = new LinkedHashSet<>();
    ...
    public void addNPECall(GenericAccessibleObject<?> call) throws IllegalArgumentException {
        NPEMethods.add(call);
    }

    public void removeNPECall(String methodKey) {
        List<GenericAccessibleObject<?>> methodList = new ArrayList<>(NPEMethods);
        
        for (GenericAccessibleObject<?> o : methodList) {
            if (o.toString().contains(methodKey)) 
                NPEMethods.remove(o);
        }
    }
...
//					}
    ...
    public GenericAccessibleObject<?> getRandomCallFor(GenericClass<?> clazz, TestCase test, int position)
        ...
        GenericAccessibleObject<?> call = Properties.NPETEST_ON ? Randomness.choice(calls) : Randomness.choice(calls);        
    ...
    public GenericAccessibleObject<?> getRandomTestCall(TestCase test)
        ...
        if (Properties.NPETEST_ON) logger.debug("SELECTED OBJECT: " + choice.toString());
    ...
    private GenericAccessibleObject<?> choose (List<GenericAccessibleObject<?>> methodList) {
        String selectedMethodKey = ClassInfo.getInstance().chooseTargetMethod();

        if (selectedMethodKey == null) return Randomness.choice(methodList);

        for (GenericAccessibleObject<?> o : methodList) {
            if (o.toString().contains(selectedMethodKey)) return o;
        }

        return Randomness.choice(methodList);
    }

    
    // generate TestCases only for the NPE-prone public methods
    public GenericAccessibleObject<?> getGuidedTestCall(TestCase test)
            throws ConstructionFailedException {

        List<GenericAccessibleObject<?>> candidateTestMethods = new ArrayList<>(NPEMethods);
        
        candidateTestMethods = filterConstructors(candidateTestMethods);

        candidateTestMethods.removeIf(o -> o.isConstructor());        

        if (candidateTestMethods.isEmpty()) {
            LoggingUtils.getEvoLogger().info("TARGET METHOD IS EMPTY");
            logger.debug("No more calls");
            // TODO: return null, or throw ConstructionFailedException?
            return null;
        }
        ...
        GenericAccessibleObject<?> choice = 
            Randomness.nextFloat() <= 0.5 ? Randomness.choice(candidateTestMethods) : choose(candidateTestMethods);
    ...
    // generate TestCases only for the NPE-prone public methods
    public GenericAccessibleObject<?> getNonTargetTestCall(TestCase test)
            throws ConstructionFailedException {

        List<GenericAccessibleObject<?>> candidateTestMethods = new ArrayList<>(testMethods);
        List<GenericAccessibleObject<?>> nonCandidateTestMethods = new ArrayList<>(NPEMethods);

        // if (!candidateTestMethods.equals(nonCandidateTestMethods))
        //     candidateTestMethods.removeAll(nonCandidateTestMethods);
    ...
    /*
     * For fuzzing Strategy... used when parsing the java source code 
     * and generates new test cases based on that code.
     */
    public GenericAccessibleObject<?> getSpecificTestCall(String targetCall, List<CtElement> params) {
        GenericConstructor genericConstructor = null;
        GenericConstructor typeCastingConstr = null;
        int resultFlag = 0;
        int typeCastFlag = 0;
        
        // Set<String> classes = TestGenerationContext.getInstance().getClassLoaderForSUT().getLoadedClasses();
        // for (String tt : classes) {
        //     logger.debug("Checking call TEst:" + tt.toString() + ";;");
        // }
        ...
        try {
            String param_typ = "";
            sut = TestGenerationContext.getInstance().getClassLoaderForSUT().loadClass(targetCall);
        
            GenericClass<?> clazz = GenericClassFactory.get(sut);

            //GenericClassFactory.
            int arr_size = params.size();

            for (Constructor<?> constructor : clazz.getRawClass().getDeclaredConstructors()) {
                resultFlag = 0;
                typeCastFlag = 0;
                
                if (constructor.getParameterCount() == arr_size) {
                    Class<?>[] check_params = constructor.getParameterTypes();
                    
                    for (int i = 0; i < arr_size; i++) {
                            
                        if (params.get(i).getValueByRole(CtRole.TYPE) == null) {
                            List<CtElement> paramCheck = params.get(i).getElements(new TypeFilter(CtFieldReferenceImpl.class));
                            if (paramCheck.size() != 0) {
                                String fieldCall = "";
                                List<CtElement> tmpType = params.get(i).getElements(new TypeFilter(CtTypeReferenceImpl.class));
                                if (tmpType.get(0).toString().contains("<nulltype>") && tmpType.size() > 1) fieldCall = tmpType.get(1).toString();
                                else fieldCall = tmpType.get(0).toString();
                                // String fieldCall = params.get(i).getElements(new TypeFilter(CtTypeReferenceImpl.class)).get(0).toString();
                                Class<?> fieldSut = TestGenerationContext.getInstance().getClassLoaderForSUT().loadClass(fieldCall);
                                for (Field ff : fieldSut.getFields()) {
                                    String fieldVar = ff.toString();
                                    String[] tmpList = fieldVar.split("[.]");
                                    String checkField = tmpList[tmpList.length - 1];
                                    if (checkField.equals(paramCheck.get(0).toString())) {
                                    //if (ff.toString().contains(paramCheck.get(0).toString())) {
                                        // logger.debug("field type check: " + ff.getAnnotatedType().getType().toString());
                                        param_typ = ff.getAnnotatedType().getType().toString();
                                        break;
                                    }
                                }
                            }
                        } else {
                            logger.debug("CtRole type is not null");
                            param_typ = params.get(i).getValueByRole(CtRole.TYPE).toString();

                            if (param_typ.equals("<nulltype>")) {
                                List<CtElement> tmpType = params.get(i).getElements(new TypeFilter(CtTypeReferenceImpl.class));
                                if (tmpType.get(0).toString().contains("<nulltype>") && tmpType.size() > 1) param_typ = tmpType.get(1).toString();
                                else param_typ = tmpType.get(0).toString();
                            }
                        }

                        logger.debug("PARAM CHECK: " + check_params[i].getTypeName() + ": " + param_typ );

                        // logger.debug("PARAM CHECKS:" + params.get(i).getValueByRole(CtRole.TYPE).toString() + ", " + param_typ) ;
                                                            
                        if (check_params[i].getTypeName().equals(param_typ) || 
                            (check_params[i].getTypeName().endsWith("InputStream") && param_typ.endsWith("InputStream"))) {
                            resultFlag++;
                        } else {

                            try {
                                Class<?> checkTyp = getClassForName(param_typ);

                                if (check_params[i].getClass().isInstance(checkTyp)) {
                                    typeCastFlag++;
                                }
                            //} catch (NullPointerException | ClassNotFoundException e) {
                            } catch (NullPointerException e) {
                                // TODO Auto-generated catch block
                                // logger.debug("CLASS NOT FOUND FOR: " + targetCall);
                                continue;
                            }
                        }
                    }

                    // if (typeCastFlag != 0 && resultFlag == arr_size) genericConstructor = new GenericConstructor(constructor, clazz);
                    // else if (resultFlag == arr_size) {
                    //     return new GenericConstructor(constructor, clazz);
                    // }
                    if (resultFlag == arr_size) {
                        genericConstructor = new GenericConstructor(constructor, clazz);
                        break;
                    }
                    else if (typeCastFlag != 0 && typeCastFlag + resultFlag == arr_size) typeCastingConstr = new GenericConstructor(constructor, clazz);
        ...
        if (genericConstructor != null) return genericConstructor;
        else return typeCastingConstr;
    }

        
    private Class<?> getClassForName(String type) {
        
        if (type.startsWith("org.evosuite.shaded.")) {
            type = type.replaceFirst("org.evosuite.shaded.","");
        }

        if (type.equals("<nulltype>")) return null;

        try {
            if (type.equals("boolean") || type.equals("java.lang.Boolean")) {
                return Boolean.TYPE;
            } else if (type.equals("byte") || type.equals("java.lang.Byte") || type.equals("byte.class")) {
                return Byte.TYPE;
