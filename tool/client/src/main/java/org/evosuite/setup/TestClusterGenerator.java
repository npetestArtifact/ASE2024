...
import org.evosuite.analysis.StaticAnalyzer;
...
public class TestClusterGenerator {
    ...
    public void generateCluster(CallGraph callGraph) throws RuntimeException, ClassNotFoundException {
        ...
        if (Properties.NPETEST_ON) {
            logger.info("Initialize Static Analaysis");
            StaticAnalyzer.getInstance().setup();
            StaticAnalyzer.getInstance().analyze();
        }
            ...
            for (Method method : TestClusterUtils.getMethods(clazz)) {
                ...
                if (TestUsageChecker.canUse(method, clazz)) {
                    ...
                    if (method.getDeclaringClass().equals(clazz)) {
                        ...
                        if (Properties.NPETEST_ON && ClassInfo.getInstance().isTargetMethod(method)) {
                            cluster.addNPECall(genericMethod);
                        }
                    }
