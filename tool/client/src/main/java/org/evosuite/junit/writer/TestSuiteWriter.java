...
public class TestSuiteWriter implements Opcodes {
    ...
    public int insertTestNPETest(TestCase test) {
    ...
    public void insertTests(List<TestCase> tests) {
        for (TestCase test : tests)
            if(Properties.NPETEST_ON) insertTestNPETest(test);
            else insertTest(test);
    }
    ...
    public List<File> writeTestSuite(String name, String directory, List<ExecutionResult> cachedResults) throws IllegalArgumentException {
        ...
        for (TestCase test : testCases) {
            ...
            logger.debug("\n");
            logger.debug("IN WRITE TEST SUITE: " + Boolean.toString(added));
            logger.debug(test.toCode());
        }
        ...
        logger.debug("Tracing results");
        for (ExecutionResult rr : results) {
            logger.debug("GET COVERED METHODS");
            logger.debug(rr.getTrace().getCoveredMethods().toString());
            logger.debug(rr.test.toCode());
        }
        ...
        if (Properties.OUTPUT_GRANULARITY == OutputGranularity.MERGED || testCases.size() == 0) {
            ...
            for (int i = 0; i < testCases.size(); i++) {
                ...
                LoggingUtils.getEvoLogger().info("GET TEST CODE");
                LoggingUtils.getEvoLogger().info(testCode.toString());
    ...
    public List<File> writeTestSuite(String name, String directory) throws IllegalArgumentException {
        ...
        if (Properties.OUTPUT_GRANULARITY == OutputGranularity.MERGED || testCases.size() == 0) {
            ...
            for (int i = 0; i < testCases.size(); i++) {
                File file = new File(directory + "/" + name + "_" + i + ".java"); // e.g., dir/Foo_ESTest_0.java
                //executor.newObservers();
                String testCode = getOneUnitTestInAFile(name, i, results);
                FileIOUtils.writeFile(testCode, file);
                content += testCode;
    ...
    protected String testToString(int number, int id, ExecutionResult result) {
        ...
        // write all statements in the test case regardless of an exception
        if (!Properties.NPETEST_ON) {
