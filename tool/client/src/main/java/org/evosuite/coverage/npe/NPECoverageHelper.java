 ...
 */
public class NPECoverageHelper {
    ...
    public static String getLine(ExecutionResult result, int exceptionPosition) {
        Throwable t = result.getExceptionThrownAtPosition(exceptionPosition);

        LoggingUtils.getEvoLogger().info("NPE COVERAGE HELPER TEST: " + getMethodIdentifier(result, exceptionPosition) + " : " + t.getMessage());
        LoggingUtils.getEvoLogger().info(t.toString());

        LoggingUtils.getEvoLogger().info("STACK TRACES");
        for (StackTraceElement ss : t.getStackTrace()) {
            LoggingUtils.getEvoLogger().info(ss.toString() + " - " + ss.getMethodName() + " - " + ss.getLineNumber());
            return ss.getMethodName()+":"+ss.getLineNumber();
        }
    ...
    public static boolean shouldSkip(ExecutionResult result, int exceptionPosition) {
        ...
        // We only consider NPE
        if (!t.getMessage().equals("null")) return true;
